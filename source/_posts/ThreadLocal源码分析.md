...
ThreadLocalMap并没有这个bug，那么它的解决办法是：在remove时，它会清除该节点之后的无用节点，以及将一些节点"往前挪"，也可能不往前挪，保证了一个优先级：一个节点本来应该放在一个数组的位置空闲了，那么这个节节点就会改变现在的位置来占有这个位置。
